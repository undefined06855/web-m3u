...
let server = Bun.serve({
    routes: {
        ...
        "/:playlist": async req => {
            ...
            let allowedModes = [ "shuffle", "date" ];
            let mode = "";
            let split = playlist.split("-");
            if (split.length > 1) {
                mode = split.pop();
            ...
            // if you edit this next line i will be very sad by the way so think about that (but go ahead... i guess...)
            ...
            if (mode == "shuffle") {
                ...
                assembler.files.sort((a, b) => Bun.file(a.path).lastModified < Bun.file(b.path).lastModified);
            } else {
                assembler.files.sort((a, b) => a.path.localeCompare(b.path));
            }
